...
public class TarefasController {
	...
	public ModelAndView inserir(@Valid Tarefa tarefa, BindingResult result, HttpServletRequest request) {
		...
		if (tarefa.getDataExpiracao() == null) {
			result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data de exprição é obrigatória");
		} else {
            SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
			if (sdf.format(tarefa.getDataExpiracao()).compareTo(sdf.format(new Date())) < 0) {
				result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida",
						"A data de exprição não pode ser anterior a data atual");
			}
		}
		if (result.hasErrors()) {
			mv.setViewName("tarefas/inserir");
